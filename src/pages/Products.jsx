...
import { Box, Grid, Card, CardHeader, CardContent, CardActions, Button, Typography, TextField, Chip, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
      ...
      <Grid container spacing={2} columns={12} mb={4}>
        {filtered.map(product => (
          <Grid item xs={12} sm={6} md={4} key={product.id} sx={{ display: 'flex', justifyContent: 'center' }}>
            <Card sx={{ width: 340, minHeight: 120, boxShadow: 3, display: 'flex', flexDirection: 'row', alignItems: 'center', p: 1 }}>
              <Box sx={{ flex: '0 0 80px', display: 'flex', alignItems: 'center', justifyContent: 'center', height: 80 }}>
                <Chip label={product.stock > 0 ? 'Disponible' : 'Agotado'} color={product.stock > 0 ? 'success' : 'error'} size="small" />
              </Box>
              <Box sx={{ flex: 1, px: 2 }}>
                <Typography variant="h6" color="primary.main" fontWeight={700} noWrap>{product.name}</Typography>
                <Typography variant="subtitle2" color="text.secondary" noWrap>Cat: <b>{product.category || 'Sin categoría'}</b></Typography>
                <Chip label={`Stock: ${product.stock}`} size="small" color="info" sx={{ mr: 1, mt: 1 }} />
                <Chip label={product.description || 'Sin descripción'} size="small" sx={{ mt: 1 }} />
                <Chip label={`PRECIO: ${formatPrice(product.price)}`} size="medium" color="error" sx={{ mt: 1 }} />
              </Box>
